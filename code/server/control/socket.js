    ...
    socket.on('cfmsg',function (data) {
        ...
        mongo.getConnection(function (db) {
            db.collection('friend').findOne({myid:ObjectId(userid),frid:ObjectId(data.to)},function (err, friend) {
                ...
                if (friend) {
                    db.collection('flog').insertOne({fid:friend._id,msg:data.msg,datetime:data.sendDate}, function (err, reply) {
                        if (err){
                            log.error(err);
                        } else {
                            log.debug(reply);
...
function savegroupLogs(userid, groupid, msg, datetime) {
    mongo.getConnection(function (db) {
        db.collection('glog').insertOne({uid:ObjectId(userid),gid:ObjectId(groupid),datetime:datetime,remark:msg});
    })
}
...
function checkGroupMenber(userid, groupid, next) {
    redis.sismember('g'+groupid,userid, function (err, reply) {
        ...
        mongo.getConnection(function (db) {
            db.collection('guser').findOne({uid:ObjectId(userid),gid:groupid}, function (err, guser) {
                if (err) {
                    log.error(err);
                    return false;
                }
